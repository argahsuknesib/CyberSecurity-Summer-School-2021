...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    private static long hashLength0to16(byte[] bArr, int i, int i2) {
        if (i2 >= 8) {
            ...
            return hashLength16(((long) i2) + ((((long) LittleEndianByteArray.load32(bArr, i)) & 4294967295L) << 3), ((long) LittleEndianByteArray.load32(bArr, (i + i2) - 4)) & 4294967295L, ((long) (i2 * 2)) - 7286425919675154353L);
        } else if (i2 <= 0) {
    ...
    private static long hashLength17to32(byte[] bArr, int i, int i2) {
        ...
        return hashLength16((LittleEndianByteArray.load64(bArr2, i3 - 16) * -7286425919675154353L) + Long.rotateRight(load64 + load642, 43) + Long.rotateRight(load643, 30), load64 + Long.rotateRight(load642 - 7286425919675154353L, 18) + load643, j);
    }
    ...
    private static long hashLength33To64(byte[] bArr, int i, int i2) {
        ...
        int i3 = i + i2;
        long load643 = LittleEndianByteArray.load64(bArr2, i3 - 8) * j;
        long rotateRight = Long.rotateRight(load64 + load642, 43) + Long.rotateRight(load643, 30) + (LittleEndianByteArray.load64(bArr2, i3 - 16) * -7286425919675154353L);
    ...
    private static long hashLength65Plus(byte[] bArr, int i, int i2) {
        ...
        long load64 = LittleEndianByteArray.load64(bArr, i) + 95310865018149119L;
        int i3 = i2 - 1;
        int i4 = i + ((i3 / 64) * 64);
        int i5 = i3 & 63;
        int i6 = (i4 + i5) - 63;
        ...
        int i7 = i;
        while (true) {
            long rotateRight = (Long.rotateRight(((load64 + j) + jArr[0]) + LittleEndianByteArray.load64(bArr2, i7 + 8), 37) * -5435081209227447693L) ^ jArr2[1];
            long rotateRight2 = (Long.rotateRight(j + jArr[1] + LittleEndianByteArray.load64(bArr2, i7 + 48), 42) * -5435081209227447693L) + jArr[0] + LittleEndianByteArray.load64(bArr2, i7 + 40);
            ...
            if (i7 == i4) {
                ...
                long rotateRight4 = (Long.rotateRight(((rotateRight3 + rotateRight2) + jArr[0]) + LittleEndianByteArray.load64(bArr2, i6 + 8), 37) * j2) ^ (jArr2[1] * 9);
                long rotateRight5 = (Long.rotateRight(rotateRight2 + jArr[1] + LittleEndianByteArray.load64(bArr2, i6 + 48), 42) * j2) + (jArr[0] * 9) + LittleEndianByteArray.load64(bArr2, i6 + 40);
                long rotateRight6 = Long.rotateRight(rotateRight + jArr2[0], 33) * j2;
                byte[] bArr3 = bArr;
                weakHashLength32WithSeeds(bArr3, i6, jArr[1] * j2, rotateRight4 + jArr2[0], jArr);
                weakHashLength32WithSeeds(bArr3, i6 + 32, rotateRight6 + jArr2[1], LittleEndianByteArray.load64(bArr2, i6 + 16) + rotateRight5, jArr2);
                long j3 = j2;
                return hashLength16(hashLength16(jArr[0], jArr2[0], j3) + (shiftMix(rotateRight5) * -4348849565147123417L) + rotateRight4, hashLength16(jArr[1], jArr2[1], j3) + rotateRight6, j3);
            }
